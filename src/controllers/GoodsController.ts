...
export default class GoodsController {
  // todo 过滤首次加入购物车的商品
  static filterGoodsItem(item: IOriginGoodsItem, options: any = {}) {
    const { unit, sideDish, taste, make, combo, count } = options
    const defaultItem: IGoodsItem = {
      ...item,
      id: nanoid(),
    ...
    if (sideDish && sideDish.length) {
      ...
      defaultItem.sideDish = sideDish.map((sideDishItem: any) => {
        return this.returnNewItem(sideDishItem, {
          items: sideDishItem.items.map((subSideDishItem: any) => {
    ...
    if (combo) {
      ...
      defaultItem.combo.items = combo.items.filter((comboItem: any) => comboItem.isSelected)
      defaultItem.combo.items = defaultItem.combo.items.map((comboItem: any) => {
        defaultItem.price = filterAmount(defaultItem.price + Number(comboItem.addPrice))

        const comboItemOptions: any = {
          SFMUnitCode
        }
        if (comboItem?.sideDish?.length && this.hasSideDish(comboItem.sideDish)) {
          ...
          comboItemOptions.sideDish = comboItem.sideDish.map((sideDishItem: any) => {
            return this.returnNewItem(sideDishItem, {
              items: sideDishItem.items.map((subSideDishItem: any) => {
                return this.returnNewItem(subSideDishItem, {
                  isBatching: 2,
   ...
   */
  static returnNewItem(item: any, options = {}) {
    return {
      ...item,
      ...options
    }
  ...
  // todo 筛选出免费的做法
  static filterMake(make: any[]) {
    return make.map(makeItem => {
      return {
        ...makeItem,
        items: makeItem.items.filter((subMakeItem: any) => {
  ...
  // todo 筛选出需要加价的做法
  static filterExtraMake(make: any[], unitName: string, options = {}) {
    const arr: any[] = []
    const peopleNum = store.getState().common.getIn(['moduleConfig', 'peopleNum'])
    make.map(makeItem => {
      makeItem.items.map((subMakeItem: any) => {
   ...
   *  同一规格:unitId
   */
  static findGoodsIndex(item: IGoodsItem, data: IGoodsItem[]): number {
    ...
    if ('sideDish' in item) verifyObject.hasSideDish = false
    if ('combo' in item) verifyObject.combo = false
    if ('taste' in item) verifyObject.taste = false
    if ('make' in item) verifyObject.make = false
  ...
  static findGoodsIndexById(id: string, data: IGoodsItem[]) {
    return data.findIndex(item => item.id === id)
  }

  static verifySubGoodsInGoods(subGoods: any[], oldSubGoods: any[]) {
    if (!subGoods || !oldSubGoods || subGoods.length !== oldSubGoods.length) return false
    const findIds = function (items: any[]) {
  ...
  static verifyComboInGoods(combo: any, oldCombo: any) {
    ...
    const findIds = function (items: any[]) {
      ...
      items.map(item => {
        ...
        if (item.subGoods) {
          item.subGoods.map((subGoodsItem: any) => {
  ...
  //    例如某个配菜需要提示必选1个菜品
  static renderSubtitleTips(item: any) {
  ...
  // todo 查找默认加入的配菜
  static findDefaultSideDish(item: any) {
    const { joinFood = [] } = item
    const originGoodsList = store.getState().common.get('originGoodsList')
    const joinArr: any[] = []
    joinFood.map((foodItem: any) => {
      const index = originGoodsList.findIndex((goodsItem: any) => goodsItem.get('foodID') === foodItem.foodID)
      if (index <= 0) return false
      const targetItem = originGoodsList.get(index).toJS()
      if (targetItem.units.length === 1) {
        joinArr.push(GoodsController.filterGoodsItem(targetItem, { unit: targetItem.units[0] }))
      }
    })
  ...
  // todo 配菜选择完成的回调
  static subGoodsSelected(options: any = {}) {
    ...
    if (limit === 'rangeLimit') {
      ...
      if (items.size < maxNum && noRepeatableSelect) {
        const isExist = items.some((oldItem: any) => oldItem.foodID === curChildrenItem.foodID)
        if (isExist) {
   ...
   */
  static getTotalPrice(unit: any, sideDish: any, combo: any, makingMethod: any) {
    let price = 0
    // 规格
  ...
  // todo 获取配菜组内相加后的价格
  static getSideDishPrice(sideDish: any[] | undefined) {
    let price = 0
    if (sideDish?.length) {
      sideDish.map(sideDishItem => {
        sideDishItem.items.map((subSideDishItem: any) => {
   ...
   * @param addNumber
   * */
  static getExtraMakePrice(make: any[], index: number, addNumber: number) {
    let price = 0
    make?.map(makeItem => {
  ...
  static getMakePrice(make: any[], type?: string) {
    const peopleNum = store.getState().common.getIn(['moduleConfig', 'peopleNum'])
  ...
  static getMakeUnit(item: any, comboItemUnit: string, type: string) {
    const peopleNum = store.getState().common.getIn(['moduleConfig', 'peopleNum'])
  ...
  // todo 判断sideDish是否存在
  static hasSideDish(sideDish: any[]) {
