...
export default class ShopPickerController {
  constructor(item = {}) {
    this.item = item;
  ...
  findFirstUnit() {
    const firstUnit = this.units?.find(unitItem => {
      return !unitItem.hasOwnProperty("total") || unitItem.total > 0;
    });
    if (firstUnit) return fromJS(firstUnit).toJS();
    return null;
  }

  // todo 验证当前选择的内容是否符合加入到购物车内
  //    例如:某些必选项,或数量范围的条件,如果不达标不允许加入购物车
  verifyAdd(sideDish, combo, taste, make) {
    const verifyObject = {};
  ...
  verifyCombo(data) {
    ...
    const length = items?.filter(subComboItem => subComboItem.isSelected).length;
    if (canSwitch === "1") {
